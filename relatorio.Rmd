...
*Concentração dos Gêneros nas Funções e Níveis de Cargo*
...
Primeiro, é importante pontuar que, para as análises mostradas a seguir, foram consideradas apenas pessoas cuja situação de trabalho era "Empreendedor ou Empregado (CNPJ)" e "Empregado (CLT)", pois são os casos mais vistos dentro das empresas e, além disso, diferentes formas de remuneração, ganhos de benefícios, etc não ajudariam muito as análises. Portanto, pessoas que trabalham como freelancer, acadêmicos, servidores públicos foram excluídos, assim como estagiários (somente pessoas devidamente empregadas foram inclusas). Também foram deixadas de lado pessoas que se identificaram com a categoria de gênero "Outros" (o objetivo é investigar possiveis diferenças entre homens e mulheres e, além disso, apenas 5 pessoas entraram nesta categoria, o que complicaria comparações com os outros gêneros).

```{r message = FALSE}

###Importando a base de dados pesquisa 2021##########################

# OBS: Neste trabalho é utilizada a linguagem de programação R

SoD2021<-readr::read_csv('../input/state-of-data-2021/State of Data 2021 - Dataset - Pgina1.csv')
`%>%`=magrittr::`%>%`
`%notin%`=Negate(`%in%`)
...
levels(df$`('P2_h ', 'Faixa salarial')`)[
  levels(df$`('P2_h ', 'Faixa salarial')`)%in%c("Menos de R$ 1.000/mês",
                                                         "de R$ 1.001/mês a R$ 2.000/mês")] <- "Menos de R$ 2.000/mês"

#Transofrmando a coluna "faixa de idade" em factor

df$`('P1_a_a ', 'Faixa idade')`=as.factor(df$`('P1_a_a ', 'Faixa idade')`)

#Criando as categorias "Acima de 50" em "faixa de idade"

levels(df$`('P1_a_a ', 'Faixa idade')`)[
  levels(df$`('P1_a_a ', 'Faixa idade')`)%in%c("50-54","55+")] <- "50+"

#Renomeando faixa salarial
df=df%>%dplyr::mutate(`('P2_h ', 'Faixa salarial')`=forcats::lvls_revalue(
...
df=df%>%dplyr::mutate(`('P1_i ', 'Área de Formação')`=forcats::lvls_revalue(
  `('P1_i ', 'Área de Formação')`, c('Saúde/Biologia, etc', 'Ciências Sociais',
                                 'TI', 'Finanças, etc', 'Estatística/ Matemática',
                                 'Marketing/Jornalismo, etc', 'Outras',
                                 'Engenharias','Química / Física'))
)

###Importando a base de dados pesquisa 2019 ##########################

SoD2019<-readr::read_csv(
    '../input/pesquisa-data-hackers-2019/datahackers-survey-2019-anonymous-responses.csv')
`%>%`=magrittr::`%>%`
`%notin%`=Negate(`%in%`)
...
pessoas2021=df%>%dplyr::count(`('P1_b ', 'Genero')`)%>%
  dplyr::mutate(perc = n/sum(n)*100)%>%.[,-2]
colnames(pessoas2021)=c('Gênero', 'Percentual %')
paste()
paste('State of Data 2021')
paste()
pessoas2021

#Pessoas por gênero - datahackers-survey-2019

pessoas2019=df2%>%dplyr::count(`('P2', 'gender')`)%>%
  dplyr::mutate(percentual = n/sum(n)*100)%>%.[,-2]
colnames(pessoas2019)=c('Gênero', 'Percentual %')
paste()
paste('Data Hackers Survey 2019')
paste()
pessoas2019
...
## Satisfação
Em termos de satisfação, poderíamos chegar a conclusão de que não há grandes diferenças no tratamento entre os gêneros na área de dados, pois os percentuais de satisfação de homens e mulheres são bastante semelhantes nos níveis de cargo e também nas funções, sendo que as maiores diferenças estão nos cargos de gestão, onde 75% das gestoras entrevistadas se mostraram satisfeitas contra 80% dos gestores homens, e na função de engenharia de dados, onde 78,6% das engenheiras mostraram satisfação contra 85% deles. 
No entanto, por se tratar de um problema social complexo que, em muitas situações, é complicado de ser percebido, se pautar apenas pela comparação de satisfação das pessoas de ambos gêneros pode não ser a melhor opção.

```{r message = FALSE}
#Satisfação por nível de cargo
paste('Satisfação x Nível de Cargo')
paste()
satisfac1=df%>%
  ...
  dplyr::filter(
    `('P2_k ', 'Você está satisfeito na sua empresa atual?')`==1)%>%.[,-3:-4]

colnames(satisfac1)<-c('Nível de Cargo', 'Gênero', 'Percentual (%)')
satisfac1

#Satisfação por atuação
paste()
paste('Satisfação x Atuação')
paste()
satisfac2=df%>%
...
Isto também acontece entre os níveis de cargo, sendo que, considerando os profissionais sênior, entre as mulheres apenas 22,1% não possuem pós-graduação, enquanto que este percentual é de mais de 35.7% entre os homens. No nível pleno, eles ganham em doutorado (0.82% x 2,94%) mas perdem em pós (38,5% x 28,4%) e mestrado (12,3% x 10,7%), ou seja, mesmo neste nível não dá para afirmar que os homens são mais escolarizados, o que seria estranho, visto que o nível pleno é o "nível do meio" ou "nível intermediário" e, como nos outros 2 níveis há "vitória feminina", algo bastante específico teria que ser usado para explicar uma eventual "vitória masculina".

 Já em relação aos gestores, no entanto, há mais equilíbrio. Entre as gestoras entrevistadas apenas 31,15% não possuem pós-graduação, enquanto que este percentual é de cerca de 35.8% entre os homens.
...
df%>%
  ...
  ggplot2::theme_void()+
  ggplot2::scale_fill_brewer(type = "seq", palette = "PiYG")+
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="left",
                 plot.title=ggplot2::element_text(face='bold.italic',
                                                  hjust = 0.5, size=13),
...
df%>%
  ...
  ggrepel::geom_label_repel(ggplot2::aes(
    ...
    fontface = 'bold',
    hjust=1,
    position = ggplot2::position_stack(vjust = .8),
    size=3.5) +
...
df%>%
  ...
  ggplot2::theme_void()+
  ggplot2::scale_fill_brewer(type = "seq", palette = "RdBu")+
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="left",
                 plot.title=ggplot2::element_text(face='bold.italic',
                                                  hjust = 0.5, size=13),
...
Entretanto, parece que os títulos das mulheres "valem menos" que o dos homens, pois considerando os 4 principais níveis escolares (bacharel, pós, mestrado e doutorado) as mulheres que os possuem estão menos presentes, proporcionalmente, nos cargos sênior e de gestão, do que os homens na mesma situação. A maior escolaridade das mulheres pode ser, também, um reflexo da necessidade delas terem que se provar mais que os homens, já que o meio social frequentemente as menospreza (e os gráficos mostrados a seguir podem ser um indício disto), forçando-as a darem sempre o melhor de si e buscando se qualificarem mais que os homens.
...
ggrepel::geom_label_repel(ggplot2::aes(label = scales::percent(Prop)),
  ...
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="left",
                 plot.title=ggplot2::element_text(face='bold.italic',hjust = 0.5, size=13),
                 axis.text.y=ggplot2::element_blank(),
                 axis.title.y=ggplot2::element_blank(),
                 axis.title.x=ggplot2::element_blank(),
                 axis.text.x =ggplot2::element_text(face='bold', size=10),
...
plot1=df%>%
  ...
  ggrepel::geom_label_repel(ggplot2::aes(
    ...
    position = ggplot2::position_stack(vjust = .5),
    size=3.5) +
  ggplot2::ggtitle('Figura 7: Experiência Dados x Gênero')+
  ggplot2::theme_void()+
  ggplot2::scale_fill_brewer(type = "seq", palette = "PRGn")+
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="left",
                 plot.title=ggplot2::element_text(face='bold.italic',
                                                  hjust = 0.5, size=13),
...
plot2=df%>%
  ...
  ggrepel::geom_label_repel(ggplot2::aes(
    ...
    position = ggplot2::position_stack(vjust = .5),
    size=3.5) +
  ggplot2::ggtitle('Figura 8: Experiência TI x Gênero')+
  ggplot2::theme_void()+
  ggplot2::scale_fill_brewer(type = "seq", palette = "BrBG")+
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="left",
                 plot.title=ggplot2::element_text(face='bold.italic',
                                                  hjust = 0.5, size=13),
...
df%>%
  ...
  ggrepel::geom_label_repel(ggplot2::aes(
    ...
    fontface = 'bold',
    hjust=1,
    position = ggplot2::position_stack(vjust = .3),
    size=3.5) +
  ggplot2::ggtitle('Figura 9: Experiência Dados x Nível de Cargo x Gênero')+
  ggplot2::theme_void()+
  ggplot2::scale_fill_brewer(type = "seq", palette = "PRGn")+
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="left",
...
df%>%
  ...
  ggrepel::geom_label_repel(ggplot2::aes(
    ...
    position = ggplot2::position_stack(vjust = .5),
    size=3.5) +
  ggplot2::ggtitle('Figura 10: Experiência TI x Nível de Cargo x Gênero')+
  ggplot2::theme_void()+
  ggplot2::scale_fill_brewer(type = "seq", palette = "BrBG")+
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="left",
...
Entre os analistas e, principalmente, cientistas de dados, há equilíbrio entre os gêneros em termos de experiência na área. Já entre os engenheiros de dados, a vantagem é ampla dos homens, pois faltam engenheiras de dados sênior, que seriam as profissionais mais experientes (Figura 2), sendo 26,79% das mulheres possuindo o mínimo de 4 anos de vivência na área,  enquanto que os homens entrevistados aparecem com 41,3%.
Considerando a experiência anterior em TI, em todas as funções, a vantagem, entre os entrevistados, é masculina, sendo que, entre os engenheiros de dados, 10,67% das mulheres possuem 6 ou mais anos de experiência enquanto que, entre os homens, este percentual é de mais de 25%. Aparentemente, por se tratar de uma função extremamente técnica, o que exigiría mais experiência em TI, há poucas mulheres sênior na engenharia de dados. 
Além disso, a função de análise de dados exige menos experiência em TI, o que pode explicar a maior proporção feminina e, como as mulheres são mais escolarizadas que os homens, talvez seria de se esperar, não fosse a questão da vivência em TI, uma maior proporção feminina em ciência de dados, já que a análise de dados é vista por muita gente (não necessariamente é verdade) como porta de entrada para a ciência de dados.
...
df%>%
  ...
  ggrepel::geom_label_repel(ggplot2::aes(
    ...
    position = ggplot2::position_stack(vjust = .5),
    size=3.5) +
  ggplot2::ggtitle('Figura 11: Experiência Dados x Atuação x Gênero')+
  ggplot2::theme_void()+
  ggplot2::scale_fill_brewer(type = "seq", palette = "PRGn")+
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="left",
...
df%>%
  ...
  ggrepel::geom_label_repel(ggplot2::aes(
    ...
    fontface = 'bold',
    hjust=1,
    position = ggplot2::position_stack(vjust = .6),
    size=3.5) +
  ggplot2::ggtitle('Figura 12: Experiência TI x Atuação x Gênero')+
  ggplot2::theme_void()+
  ggplot2::scale_fill_brewer(type = "seq", palette = "BrBG")+
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="left",
...
df%>%
  ...
  ggrepel::geom_label_repel(ggplot2::aes(
    ...
    fontface = 'bold',
    hjust=1,
    position = ggplot2::position_stack(vjust = .4),
    size=3.5) +
  ggplot2::labs(title = 'Figura 16: Faixa Salarial x Nível de Cargo x Gênero')+
  ggplot2::theme_void()+
  ggplot2::scale_fill_brewer(type = "seq", palette = "PuOr")+
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="left",
                 plot.title=ggplot2::element_text(face='bold.italic',
                                                  hjust = 0.5, size=13),
...
df%>%
  ...
  ggrepel::geom_label_repel(ggplot2::aes(
    ...
    position = ggplot2::position_stack(vjust = .5),
    size=3.5) +
  ggplot2::labs(title = 'Figura 18: Faixa Salarial Engenheiros de Dados Júnior/Pleno')+
  ggplot2::theme_void()+
  ggplot2::scale_fill_brewer(type = "seq", palette = "PuOr")+
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="left",
...
colnames(situacao)=c('Gênero','Situação de Trabalho', 'Percentual %')
situacao

#Situação de trabalho por gênero e atuação
df%>%
  dplyr::count(`('P4_a ', 'Atuacao')`,`('P1_b ', 'Genero')`,
               `('P2_a ', 'Qual sua situação atual de trabalho?')`) %>%
  dplyr::group_by(`('P1_b ', 'Genero')`,`('P4_a ', 'Atuacao')`) %>%
  ...
  ggrepel::geom_label_repel(ggplot2::aes(
    ...
    position = ggplot2::position_stack(vjust = .5),
    size=3.5) +
  ggplot2::ggtitle('Figura 19: Situação x Atuação x Gênero')+
  ggplot2::theme_void()+
...
df%>%
  ...
  ggrepel::geom_label_repel(ggplot2::aes(
    ...
    position = ggplot2::position_stack(vjust = .5),
    size=3.5) +
  ggplot2::ggtitle('Figura 20: Situação x Nível de Cargo x Gênero')+
  ggplot2::theme_void()+
  ggplot2::scale_fill_brewer(type = "seq", palette = "Blues")+
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="left",
