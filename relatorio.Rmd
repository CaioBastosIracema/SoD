...
Considerando a experiência anterior em tI, em todas as funções, a vantagem, entre os entrevistados, é masculina, sendo que, entre os engenheiros de dados, 10,67% das mulheres possuem 6 ou mais anos de experiência enquanto que, entre os homens, este percentual é de mais de 25%.
```{r}
...
Como dito anteriormente, as mulheres da área de dados estão ganhando menos nos cargos sênior e de gestão. Em partes, isso pode ter relação com a maior concentração de mulheres na função que, segundo o relatório da State of Data Brazil 2021 - Figura 28, é a que menos paga (análise de dados) em todos os níveis de cargo, principalmente no nível sênior, com mais de 61% das profissionais dete nível estando nessa função contra menos de 42% dos homens sênior, além da menor concentração delas no nível sênior da engenharia de dados (que, segundo a mesma análise do relatório, é a que melhor paga no nível sênior).
...
Quando passamos a considerar apenas profissionais sem experiência prévia em TI, o balanceamento salarial entre os 2 gêneros aumenta consideravelmente, indicando que o fato das áreas de TI, incluindo a área de dados, terem sido praticamente restritas aos homens produz consequências ainda hoje.

```{r}
df%>%
  dplyr::filter(`('P4_a ', 'Atuacao')`%notin%c('Outra', 'Gestor') & 
                  `Experiência na área de TI`=='Sem experiência')%>%
  ...
  ggrepel::geom_label_repel(ggplot2::aes(
    ...
    position = ggplot2::position_stack(vjust = .5),
    size=3.5) +
  ggplot2::labs(title = 'Figura 21: Faixa Salarial x Gênero, Sem experência em TI')+
  ggplot2::theme_void()+
  ggplot2::scale_fill_brewer(type = "seq", palette = "YlOrRd")+
  ggplot2::xlab('Gênero')+
  ggplot2::theme(legend.position="bottom",
...
A maior concentração de mulheres na atuação de análise de dados, função, entre as analisadas neste relatório, com os rendimentos mais baixos, embora tenha alguma influência e, com toda a certeza, deva ser melhor investigada, não explica totalmente o fato das mulheres sênior ganharem menos que os homens sênior, pois, além da proporção de engenheiras de dados sênior consideravelmente menor que a dos homens na mesma situação, as cientistas de dados sênior do gênero feminino parecem ganhar menos que os do masculino. Além disso, o gênero feminino, em relação ao masculino, além de estar menos concentrado nos cargos de gestão, possui menores salários, de uma maneira geral. O fator que mais parece explicar esse fenômeno é a considerável derrota das mulheres para os homens em termos de experiências prévias na área de TI, resultado de décadas de exclusão do gênero feminino neste tipo de área. Provavelmente, isso explica, em partes, o por quê das mulheres terem dificuldade em assumir cargos de gerencia ou de liderança, resultando na permanência dos maiores salários nas mãos homens, embora outros fatores devam existir, pois este não é um problema exclusivo da área de dados ou de qualquer outra relacionada a TI, como indica o estudo do IBGE anteriormente mencionado.

É preciso destacar a vitória das mulheres em termos de nível de escolaridade que, segundo o IBGE, também não é exclusividade da área dos dados.
Em nenhum dos níveis de cargo da para dizer que os homens são mais escolarizados e o mesmo vale em relação às funções, incluindo a engenharia de dados, onde, mesmo elas sendo mais novas e menos experientes que os homens, possuem, pelo menos, o nível de  pós-graduação em maior proporção que os homens. No entanto, mulheres têm mais dificuldade em assumir cargos mais importantes que homens do mesmo nível escolar.

A menor proporção de mulheres no nível sênior na engenharia de dados também precisa ser investigada. É possível que a falta de experiência prévia em TI tenha alguma influência nisso, mas, aparentemente, há escassez de mulheres mais experientes, de uma maneira geral, entre as engenheiras, pois elas possuem, consideravelmente, menos experiência na área de dados em comparação aos engenheiros (o que não acontece dentro das outras atuações), além das mulheres serem mais novas que os homens na engenharia de dados.

Dominada pelo gênero masculino, com mais de 80% de seus profissionais se indentificando com este gênero, a área de dados, embora ofereça ótimas oportunidades para as mulheres, com muitas ganhando mais de R$ 8 mil de salário, parece refletir os problemas oriundos da desigualdade de gênero que o Brasil enfrenta. 
